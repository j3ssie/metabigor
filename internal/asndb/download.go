...
const (
	dbURL  = "https://github.com/iplocate/ip-address-databases/raw/refs/heads/main/ip-to-asn/ip-to-asn.csv.zip"
	dbDir  = ".metabigor"
	dbFile = "ip-to-asn.csv"
)

// DBPath returns the path to the local ASN database.
func DBPath() string {
...
func EnsureLoaded() (*DB, error) {
	...
	if _, err := os.Stat(path); os.IsNotExist(err) {
		output.Info("ASN database not found at %s, extracting from embedded data ...", path)
		if err := ExtractEmbedded(); err != nil {
			output.Warn("Embedded extraction failed: %v — downloading from GitHub ...", err)
			if err := Download(); err != nil {
				return nil, fmt.Errorf("auto-download ASN database: %w", err)
			}
		}
	}
	return Load(path)
...
func ExtractEmbedded() error {
	...
	data := public.ASNDB
	if len(data) == 0 {
		return fmt.Errorf("embedded ASN database is empty")
	}

	return unzipCSV(data, filepath.Join(dir, dbFile))
}

// Download fetches the latest ASN database from GitHub and stores it locally.
func Download() error {
	home, err := os.UserHomeDir()
	if err != nil {
		return fmt.Errorf("get home dir: %w", err)
	...
	output.Info("Downloading ASN database from GitHub (iplocate/ip-address-databases) ...")
...
func unzipCSV(zipData []byte, dest string) error {
	...
	for _, f := range zr.File {
		...
		output.Good("ASN database saved to %s (%d MB)", dest, n/1024/1024)
		return nil
	}
